...
def main():
    ...
    scene = harness.load_scene("menu-help")
    sd = harness.Sdmc(args.sdmc)
    os.makedirs(args.out, exist_ok=True)
    captures = [f"menu_help_{i}_{kind}.ppm" for i in range(scene["cases"])
    ...
    try:
        ...
        for i in range(scene["cases"]):
            stats = harness.diff_images(os.path.join(args.out, f"menu_help_{i}_menu.png"),
                                        os.path.join(args.out, f"menu_help_{i}_help.png"),
                                        scene["regions"])
            ok &= harness.report(f"menu help case {i}", stats, scene["ab_expect"])
