...
bool impl3dsInitialize()
{
	...
	int vbo_scene_obj_size = (int)gpu3dsGetNextPowerOf2(sizeof(STileVertex) * MAX_VERTICES_OBJ * 2);
	...
	SVertexListInfo listInfos[] = {
		...
		{ VBO_SCENE_OBJ, vbo_scene_obj_size, sizeof(STileVertex), 2, { {GPU_SHORT, 3}, {GPU_SHORT, 2} } },
...
void impl3dsSceneRender(bool firstFrame, bool paused) {
	...
	{
		// crop always mirrors the shift mode (issue #65): rounding is
		// monotonic, so in discrete mode it covers every displacement
		edgeCropSrc = GPU3DS.stereoMaxAbs * (iod / gpu3dsGetIODBase()) * STEREO_PARALLAX_SCALE;
		if (settings3DS.StereoShiftMode == 0)
			edgeCropSrc = roundf(edgeCropSrc);
	}
