...
static bool (*g_status_override)(uint8_t *value) = nullptr;
void msu1_set_status_override(bool (*override_fn)(uint8_t *value)) { g_status_override = override_fn; }
...
uint8_t S9xMSU1ReadPort(uint8_t port)
{
    ...
    if (port == 0) {
        // a replay substitutes the recorded status; live reads feed the tape
        if (g_status_override && g_status_override(&value))
            return value;
        if (g_status_read_hook)
